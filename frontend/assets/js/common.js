   ...
   // jQuery( document ).on( 'elementor/popup/show', () => {
                    ...
                    for (var i = 0; i < countAnsvers; i++) {
                        ...
                        if (responseJSON['a'][i]['answer'] == 'scroll') {
                            ans = '<div class="question-el js-resize-width slider-price-wrap" style="background: none;width: 100%;">' +
                                ...
                                '<div><div style="float:left">0р</div><div style="float:right">50000000р</div></div>' +
                                '</div>';
                            ...
                            if (ans == 'freetext') {
                                ans = '<input type="text" name="g" placeholder="Другое" class="quiz-custom text text-input-other" data-next="' + ans + '" >';
                                nextBtn.data('next', responseJSON['a'][i]['nextquestion']);
        ...
        function nextQestion(id) {
            if (curentPosition == null || curentPosition == -1) {
                numCurrent++;
                titleQuestion.html('Оставте ваши контакты для связи с Вами');

                quizBodyBontent.empty();

                quizBodyBontent.append(
                    '<div class="body"><div class="input-container"><label>ВВЕДИТЕ ИМЯ' +
                    '</label><input class="input-agent" name = "agent-name" id ="agent-name"  placeholder="Имя"/>' +
          ...
          var nextq = $(this).data('next');
            ...
            if (nextq.length > 0) {
                ...
                if($('.checkbox > label > input:checked').length > 0)
                {
                    ...
                    test[curentPosition] = [currentQuestionTitle, str+' Другое: '+ other];
                ...
                success: function (response) {
                    if (response == 1) {
                        ...
                        $.alert({
                            ...
                            content: 'Для удобства работы с запросом, наши специалисты, свяжутся с Вами в ближайшее время, чтобы уточнить особые пожелания.',
                            theme: 'material',
                        });
                    }
                }
